package com.example.hotelmanagementsystem.ui.activities.admin;
            ...
            public void onClick(View v) {
                ...
                if (adminId.length() > 0 && adminEmail.length() > 0 && adminNewPassword.length() > 0 && adminNewPassword.length() > 0 && adminRetypeNewPassword.length() > 0) {
                    if (!adminNewPassword.equals(adminRetypeNewPassword)) {
                        Toast.makeText(AdminForgotPasswordActivity.this, "New passwords and retype new passwords are not the same", Toast.LENGTH_SHORT).show();
                    } else {
                            ...
                            callback.enqueue(new Callback<String>() {
                                ...
                                public void onResponse(Call<String> call, Response<String> response) {
                                    ...
                                    if (res.trim().equals("ADMIN_CHANGE_PASSWORD_SUCCESSFUL")) {
                                        Toast.makeText(AdminForgotPasswordActivity.this, "Password Changed Successfully", Toast.LENGTH_SHORT).show();
                                        Intent intent = new Intent(AdminForgotPasswordActivity.this, AdminLoginActivity.class);
                                        startActivity(intent);
                                        finish();
                                    } else if (res.trim().equals("ADMIN_CHANGE_PASSWORD_FAILED")) {
                                        Toast.makeText(AdminForgotPasswordActivity.this, "Admin Email Or ID Incorrect", Toast.LENGTH_SHORT).show();
                                    } else {
                                        Toast.makeText(AdminForgotPasswordActivity.this, "Something wrong", Toast.LENGTH_SHORT).show();
                                    }
                                }
                    ...
                    Toast.makeText(AdminForgotPasswordActivity.this, "Please enter complete information!", Toast.LENGTH_SHORT).show();
                }
            }
        });
